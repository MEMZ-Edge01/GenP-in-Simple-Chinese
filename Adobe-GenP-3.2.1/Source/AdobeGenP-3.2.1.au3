...
Func MainGui()
	...
	_GUICtrlListView_AddColumn($idListview, "点击这里以折叠所有组", 532, 2)
	...
	$idButtonCustomFolder = GUICtrlCreateButton("路径", 10, 630, 80, 30)
	GUICtrlSetTip(-1, "选择你安装Adobe软件路径 -> 点击“搜索” -> 点击“破解”")
	GUICtrlSetImage(-1, "imageres.dll", -4, 0)
	GUICtrlSetResizing(-1, $GUI_DOCKAUTO)

	$idButtonSearch = GUICtrlCreateButton("搜索", 110, 630, 80, 30)
	GUICtrlSetTip(-1, "让GenP自动找到选择路径下你安装的软件")
	GUICtrlSetImage(-1, "imageres.dll", -8, 0)
	GUICtrlSetResizing(-1, $GUI_DOCKAUTO)

	$idButtonStop = GUICtrlCreateButton("停止", 110, 630, 80, 30)
	GUICtrlSetState(-1, $GUI_HIDE)
	GUICtrlSetTip(-1, "停止搜索")
	GUICtrlSetImage(-1, "imageres.dll", -8, 0)
	GUICtrlSetResizing(-1, $GUI_DOCKAUTO)

	$idBtnDeselectAll = GUICtrlCreateButton("选择", 210, 630, 80, 30)
	GUICtrlSetState(-1, $GUI_DISABLE)
	GUICtrlSetTip(-1, "选择/取消选择所有软件")
	GUICtrlSetImage(-1, "imageres.dll", -76, 0)
	GUICtrlSetResizing(-1, $GUI_DOCKAUTO)

	$idBtnCure = GUICtrlCreateButton("破解", 305, 630, 80, 30)
	GUICtrlSetState(-1, $GUI_DISABLE)
	GUICtrlSetTip(-1, "破解所有选择文件")
	GUICtrlSetImage(-1, "imageres.dll", -102, 0)
	GUICtrlSetResizing(-1, $GUI_DOCKAUTO)

	$idBtnBlockPopUp = GUICtrlCreateButton("阻止弹窗", 405, 630, 80, 30)
	GUICtrlSetTip(-1, "通过创建Windows防火墙规则阻止未经许可的弹出框")
	GUICtrlSetImage(-1, "imageres.dll", -101, 0)
	GUICtrlSetResizing(-1, $GUI_DOCKAUTO)

	$idBtnRestore = GUICtrlCreateButton("还原", 405, 630, 80, 30)
	GUICtrlSetState(-1, $GUI_HIDE)
	GUICtrlSetTip(-1, "还原到以前的文件（取消破解）")
	GUICtrlSetImage(-1, "imageres.dll", -113, 0)
	GUICtrlSetResizing(-1, $GUI_DOCKAUTO)

	$idBtnRunasTI = GUICtrlCreateButton("管理员运行", 505, 630, 80, 30)
	GUICtrlSetImage(-1, "imageres.dll", -74, 0)
	If (@UserName = "SYSTEM") Then
		GUICtrlSetState(-1, $GUI_DISABLE)
	EndIf
	GUICtrlSetTip(-1, "以管理员身份运行GenP")
	GUICtrlSetResizing(-1, $GUI_DOCKAUTO)
...
Func FillListViewWithInfo()
	...
	_GUICtrlListView_AddSubItem($idListview, 0, "", 1)
	_GUICtrlListView_AddSubItem($idListview, 1, "想要在默认路径破解所有Adobe应用:", 1)
	_GUICtrlListView_AddSubItem($idListview, 2, "请点击“搜索”和“破解”", 1)
	_GUICtrlListView_AddSubItem($idListview, 3, "默认路径是 - C:\Program Files", 1)
	_GUICtrlListView_AddSubItem($idListview, 4, '-------------------------------------------------------------', 1)
	_GUICtrlListView_AddSubItem($idListview, 5, "搜索后一些产品可以被破解", 1)
	_GUICtrlListView_AddSubItem($idListview, 6, "要选择/取消选择要打补丁的产品，请在产品组上左键单击", 1)
	_GUICtrlListView_AddSubItem($idListview, 7, "要选择/取消选择单个文件，请右键单击该文件", 1)
	_GUICtrlListView_AddSubItem($idListview, 8, '-------------------------------------------------------------', 1)
	_GUICtrlListView_AddSubItem($idListview, 9, "更新了啥？", 1)
	_GUICtrlListView_AddSubItem($idListview, 10, "能将2019年版本的应用程序补丁到当前和未来的版本", 1)
	_GUICtrlListView_AddSubItem($idListview, 11, "自动搜索和补丁在选定的文件夹", 1)
	_GUICtrlListView_AddSubItem($idListview, 12, "新的补丁逻辑。用于Windows防火墙的“未经许可的弹出窗口”拦截程序", 1)
	_GUICtrlListView_AddSubItem($idListview, 13, "汉化：MEMZ_Edge", 1)
	_GUICtrlListView_AddSubItem($idListview, 14, '-------------------------------------------------------------', 1)
	_GUICtrlListView_AddSubItem($idListview, 15, "已知问题:", 1)
	_GUICtrlListView_AddSubItem($idListview, 16, "InDesign 和 InCopy 会有很高的CPU利用", 1)
	_GUICtrlListView_AddSubItem($idListview, 17, "Animate 在退出登录时会在首页出现显示问题", 1)
	_GUICtrlListView_AddSubItem($idListview, 18, "Acrobat, XD, Lightroom Classic 在退出登录时只会部分工作", 1)
	_GUICtrlListView_AddSubItem($idListview, 19, "Premiere Rush, Lightroom Online, Photoshop Express", 1)
	_GUICtrlListView_AddSubItem($idListview, 20, "不会被完全破解", 1)
	_GUICtrlListView_AddSubItem($idListview, 21, '-------------------------------------------------------------', 1)
	_GUICtrlListView_AddSubItem($idListview, 22, "一些应用程序需要Creative Cloud和强制登录", 1)
...
Func FillListViewWithFiles()
	...
	If UBound($FilesToPatch) > 0 Then
		...
		MemoWrite(@CRLF & UBound($FilesToPatch) & " 个文件在 " & Round(TimerDiff($timestamp) / 1000, 0) & " 秒内在" & @CRLF & "---" & @CRLF & $MyDefPath & @CRLF & "---" & @CRLF & "被找到，点击破解按钮")
		...
		$fFilesListed = 1
	Else
		MemoWrite(@CRLF & "没有东西被找到在下面路径" & @CRLF & "---" & @CRLF & $MyDefPath & @CRLF & "---" & @CRLF & "等待用户操作")
